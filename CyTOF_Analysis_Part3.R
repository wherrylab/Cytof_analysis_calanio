...
## It details one way to process datas generated by cytofkit package on CyTOF datas
...
###### Sasi - can you create a loop above, so that the lm is runned on ALL the clusters?
###### And eventually a readout that tells you which clusters have a p<0.05? and the direction of the effect (up or down as compared to ref group = estimate pos or neg)
###### That would simplify the heatmap step

# And/or you can create a heatmap of percentages on significant clusters
# For this, annotate the columns
...
for(i in 1:ncol(Pheno)){
  ...
  plt
  ggsave(paste0("Plot per cluster",names(Pheno)[i], ".pdf"), path=".", plt, width = 5, height = 5)
}
