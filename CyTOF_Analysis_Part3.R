## This script details one way to process data generated by cytofkit package on CyTOF data
...
pheatmap(mat = t(Pheno), color=seq.brew, ascale = "row", cluster_rows = TRUE, cluster_cols = F, filename = "./heatmap_all_clusters.pdf", width = 10, height = 5)
# Or - recommended - you can generate the heatmap on selected significant clusters
# Annotate the columns first
Pheno_df=data.frame(t(Pheno))
Pheno_sig=Pheno_df[,c(paste0("cluster_",c(2,13,17)))]
...
pheatmap(mat = Pheno_sig, fontsize=7, annotation_col = annotation_c, annotation_colors = anno_colors, color=seq.brew, scale = "row", cluster_rows = TRUE, cluster_cols = TRUE,  filename = "./heatmap_selected_clusters_annotated.pdf", width = 6, height = 5)
